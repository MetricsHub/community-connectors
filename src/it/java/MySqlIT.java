...
public class MySqlIT {
	...
	static void setUp() throws Exception {
		...
		final JdbcConfiguration jdbcConfiguration = JdbcConfiguration.builder()
				.hostname("any").username("any").password("any".toCharArray()).type("mysql").timeout(120L).build();

		final Map<Class<? extends IConfiguration>, IConfiguration> configurations = new HashMap<>();
		configurations.put(JdbcConfiguration.class, jdbcConfiguration);
		...
		extensionManager = ExtensionManager.builder().withProtocolExtensions(Collections.singletonList(new JdbcExtension())).build();	}
