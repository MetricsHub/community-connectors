...
translations: # <object>
  <table-key>: # <object>
    <key>: # <string>
```
## Creating emulation integration tests

To replay a connector in emulation mode by writing a corresponding integration test in the
project `community-connectors`, we need first to generate the recorded data (sources and criteria).
Then, we use all those recorded data in the corresponding IT resources folder in order to emulate the connector(replay it).

### Generating recorded data

To generate recorded sources and criteria, we use metricshub CLI with the `--record` option.

**Example**

```bash
metricsHub babbage -t linux --ssh-username userName --ssh-password userPassword \
-c +Linux --record
```

The recorded data is generated in the default output directory `/logs` under MetricsHub installation folder
(same level as `app` and `config`)

![Record default folder](../images/recordFolder.png)

The generated record data consists of both connector sources and criteria. 

![Record output](../images/recordOutput.png)

### IT resources configuration

The IT resources folder contains 3 subfolders:

* `config`: Contains MetricsHub configuration file `metricshub.yaml`
* `emulation`: Contains the recorded sources and criteria generated by MetricsHub CLI
* `expected`: Contains the expected Json result of the IT

ℹ️ Each subfolder of the resources directory `src/it/resources` should be
named like the connector name (case-insensitive). For instance, for `linux` connector, the corresponding
resources subfolder is called `Linux`

#### Setting up the emulation folder

After generating recorded sources and criteria files, we need to add them to the subfolder of 
the IT resources folder called `emulation` of the corresponding IT.

**Example for Linux IT:**

![Emulation files](../images/emulationFiles.png)

ℹ️ Please note that the developer should rename the emulation files.
The files names of the emulation files should start with `localhost-` even the generated
files used another host name.

#### Setting up the config folder

The `config` folder contains the `metricshub.yaml` file used to replay the connector.
It contains a minimal configuration.
Here is an example for Linux IT:
...
resources:
  localhost:
    ...
    connectors: ["+Linux"]
```
#### Setting up the expected folder

The expected folder contains `expected.json` which is the expected IT result. It corresponds to
the final JSON object of the telemetry manager after running all the strategies by MetricsHub in
emulation mode.

ℹ️ To generate `expected.json` content, developers should retrieve in debug
mode the value of `telemetryManager.toJson()` in the method `verifyExpected` in `AbstractITJob` class.

#### Adding the connector name to the parametrized IT

In Java code, developers should just add the connector name of the connector they want to test.
Here is how other connectors were added to the current parametrized test:
...
ℹ️ EmulationITBase class extends parent class AbstractITJob to use many utility methods like
`verifyExpected` and `withRecordData`.
