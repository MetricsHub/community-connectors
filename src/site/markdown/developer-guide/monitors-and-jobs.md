...
| `executionOrder` | all | No | Array of source names forcing a sequential order. By default the engine orders sources by their `${source::...}` dependencies and may run independent sources in parallel; use `executionOrder` only when a dependency is invisible to the engine. |
